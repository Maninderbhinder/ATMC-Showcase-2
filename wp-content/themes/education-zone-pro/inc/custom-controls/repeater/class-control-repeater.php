...
if( ! class_exists( 'Rara_Control_Repeater' ) ) {
    ...
    class Rara_Control_Repeater extends WP_Customize_Control {
    			...
    			foreach ( $this->fields as $field ) {
    				if ( isset( $field['type'] ) ){
    					if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_style( 'font-awesome', get_template_directory_uri(). '/assets/fonts'. $unminify .'/font-awesome'. $suffix .'.css', array(), '4.7' );
    					}
    				}
    			}
    		}
            wp_enqueue_script( 'repeater', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'repeater-css', get_template_directory_uri() . '/inc/custom-controls/repeater/repeater.css', null );
    	}
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'education-zone-pro' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
        ...
        function get_icon_list(){
    	    $icons = include wp_normalize_path( get_template_directory() . '/inc/custom-controls/fontawesome.php' ); 
            if( $icons ){ ?>
