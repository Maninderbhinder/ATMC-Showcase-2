...
// register education_zone_pro_Featured_Post widget
function education_zone_pro_register_featured_post_widget() {
    register_widget( 'education_zone_pro_Featured_Post' );
}
add_action( 'widgets_init', 'education_zone_pro_register_featured_post_widget' );
 
 /**
 * Adds education_zone_pro_Featured_Post widget.
 */
class education_zone_pro_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'education_zone_pro_featured_post', // Base ID
			__( 'RARA: Featured Post', 'education-zone-pro' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'education-zone-pro' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
        ...
        $read_more      = ! empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'education-zone-pro' );
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'education-zone-pro-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
        ...
        $postlist[0] = array(
    		'value' => 0,
    		'label' => __( '--choose--', 'education-zone-pro' ),
    	);
        ...
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'education-zone-pro' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'education-zone-pro' ); ?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'excerpt_char' ) ); ?>"><?php esc_html_e( 'Excerpt Character', 'education-zone-pro' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'excerpt_char' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'excerpt_char' ) ); ?>" type="number" step="1" min="180" max="500" value="<?php echo esc_attr( $excerpt_char ); ?>" />
		</p>
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = ! empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'education-zone-pro' );
...
} // class education_zone_pro_Featured_Post