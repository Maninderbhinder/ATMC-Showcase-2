...
// register education_zone_pro_Author_Post widget
function education_zone_pro_register_author_post_widget() {
    register_widget( 'education_zone_pro_Author_Post' );
}
add_action( 'widgets_init', 'education_zone_pro_register_author_post_widget' );
 
 /**
 * Adds education_zone_pro_Author_Post widget.
 */
class education_zone_pro_Author_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'education_zone_pro_author_post', // Base ID
			__( 'RARA: Author Post', 'education-zone-pro' ), // Name
			array( 'description' => __( 'A Author Post Widget display other posts written by the author. Will appear on single posts only.', 'education-zone-pro' ), ) // Args
		);
	}
    						...
    						<div class="entry-header">
    							...
    							<?php if( $show_postdate ){?>
                                    <div class="entry-meta">
                                        <?php 
                                        printf( '<span><i class="fa fa-calendar-o"></i><a href="%1$s" rel="bookmark"><time datetime="%2$s">%3$s</time></a></span>',esc_url( get_permalink() ), esc_attr( get_the_date( 'Y-m-d' ) ), esc_html( get_the_date( 'Y-m-d ' ) ) ); ?>
                                    </div>
                                <?php }?>
    						</div>
                        </article>
...
} // class education_zone_pro_Author_Post 